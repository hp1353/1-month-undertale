...
HRESULT stage21::init()
{
	_player->getPlayer()->loadstagenum = 21;
	...
	_player->getPlayer()->SaveStageNum = 21;
	IMAGEMANAGER->addImage("stage21", "image/undertale/stage/stage21.bmp", 820, 962, false, false);
	...
	else
	{
		...
		_player->getPlayer()->y = 850;
		_camera->getCam()->x = 0;
		_camera->getCam()->y = 484;
		_camera->getCam()->overviewX = true;
		_camera->getCam()->overviewY = true;
	}
	
	if (_player->getPlayer()->stage21clear)
	{

	}
	else
	{
		nextdoor = RectMake(800, 340, 50, 50);

		blue = RectMake(320, 750, 70, 50);
		yellow = RectMake(210, 325, 100, 60);
		red = RectMake(540, 250, 100, 70);
		text = RectMake(400, 150, 40, 80);
		dooropen = false;
		heartcontrol = 150;
	...
	IMAGEMANAGER->addImage("trapover4", "image/undertale/object/trapover.bmp", 40, 80, false, false);
	return S_OK;
}
...
void stage21::update()
{
	...
	if (textnum != 0)_player->getPlayer()->stage21nopillar = true;
	else _player->getPlayer()->stage21nopillar = false;

	if (!dooropen)loadmark = RectMake(700, 320, 40, 80);
	if (dooropen)loadmark = RectMake(0, 0, 0, 0);
}

void stage21::render()
{
	IMAGEMANAGER->findImage("stage21")->render(getMemDC());

	if (dooropen)
	{
		IMAGEMANAGER->findImage("trapover4")->render(getMemDC(), 700, 320);
	}
...
void stage21::TextControlUpdate()
{
	tcount++;
	if (tcount % 3 == 0)
	{
		...
		if (textnum == 2 && tindex > 60)
		{
			if (heartcontrol == 150)
			{
				if (_player->getPlayer()->pressZ)
				{
					...
					dooropen = true;
					_player->getPlayer()->stage21clear = true;
				}
			}
			if (heartcontrol == 280)
			{
...
void stage21::TextControl()
{
	...
	HANDLE hInputFile;
	if (textnum == 1)
		hInputFile = CreateFile(_T("text/stage21/text1.txt"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, 0);
	if (textnum == 2)
		hInputFile = CreateFile(_T("text/stage21/text2.txt"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, 0);
	if (textnum == 3)
		hInputFile = CreateFile(_T("text/stage21/text3.txt"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, 0);
