...
void aStar::update()
{
	if ( _aStarState == ASTAR_STATE_END )        //아직 배치가 끝나지 않았다면
	{
		tileComposition();                               //타일 배치
	}

	//시작지점 도착지점 설치되어 있고, 스페이스 누르면 타일 초기화
	if ( KEYMANAGER->isOnceDown( VK_SPACE ) && _startPointSet && _endPointSet && _aStarState == ASTAR_STATE_END ) { tileInitializing(); }
...
void aStar::render()
{
	...
	TextOut( getMemDC(), 15, 100 + 15, "1 : EMPTY", strlen( "1 : EMPTY" ) );
	TextOut( getMemDC(), 15, 100 + 35, "2 : START", strlen( "2 : START" ) );
	TextOut( getMemDC(), 15, 100 + 55, "3 : END", strlen( "3 : END" ) );
	TextOut( getMemDC(), 15, 100 + 75, "4 : WALL", strlen( "4 : WALL" ) );
	...
	TextOut( getMemDC(), 15, 100 + 120, str, strlen( str ) );
...
{
	if ( KEYMANAGER->isOnceDown( '1' ) ) { _selectedType = TILE_TYPE_EMPTY; _selectedTypeColor = RGB( 255, 255, 255 ); }
	else if ( KEYMANAGER->isOnceDown( '2' ) ) { _selectedType = TILE_TYPE_START; _selectedTypeColor = RGB( 100, 255, 100 ); }
	else if ( KEYMANAGER->isOnceDown( '3' ) ) { _selectedType = TILE_TYPE_END;   _selectedTypeColor = RGB( 255, 100, 100 ); }
	else if ( KEYMANAGER->isOnceDown( '4' ) ) { _selectedType = TILE_TYPE_WALL;  _selectedTypeColor = RGB( 100, 100, 255 ); }
